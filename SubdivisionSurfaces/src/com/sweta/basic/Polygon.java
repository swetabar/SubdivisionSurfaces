...
public class Polygon {
	...
	public static class Edge implements Comparable<Edge> {
		...
		private final MyHashSet<Point> ends = new MyHashSet<Point>();

		public Edge(final Point endA, final Point endB) {
			if (endA == null || endB == null)
				throw new NullPointerException();
			if (endA.equals(endB))
				throw new IllegalArgumentException("Invalid edge! endA="
						+ endA.toString() + " endB=" + endB.toString());
			this.ends.add(endA);
			this.ends.add(endB);
		}
		...
		public int hashCode() {
			...
			int result = 1;
			result = prime * result
					+ (this.ends == null ? 0 : this.ends.hashCode());
			return result;
		}
		...
		public Point midPoint() {
			...
			return Point
					.linearCombination((Point) arr[0], (Point) arr[1], 0.5f);
		}
	...
	private final MyHashSet<Edge> edges = new MyHashSet<Edge>();

	private final Vector normal;

	private final Color color;

	public Polygon(final ArrayList<Point> argPoints,
			final MyHashSet<Edge> argEdges, final Vector argNormal,
			final Color argColor) {
		if (argPoints == null || argEdges == null || argNormal == null)
			throw new NullPointerException(
					"Non-null normal and sets of points and edges required!");
		final MyHashSet<Point> edgeEnds = Polygon.validateEdges(argEdges);
		if (edgeEnds == null)
			throw new IllegalArgumentException("Invalid set of edges!");
		final MyHashSet<Point> pointsSet = new MyHashSet<Point>();
		pointsSet.addAll(argPoints);
		if (!pointsSet.equals(edgeEnds))
			throw new IllegalArgumentException(
					"Invalid set of points and edges!");
	...
	private static MyHashSet<Point> validateEdges(final MyHashSet<Edge> edges) {
		...
		final HashMap<Point, Integer> pointDegrees = new HashMap<Point, Integer>();
		...
		final MyHashSet<Integer> degreeSet = new MyHashSet<Integer>();
	...
	public String toString() {
		...
		if (this.color != Color.DEFAULT)
			builder.append("d " + this.color.x + " " + this.color.y + " "
					+ this.color.z + "\n");
	...
	public int hashCode() {
		...
		int result = 1;
		result = prime * result
				+ (this.getPoints() == null ? 0 : this.getPoints().hashCode());
		result = prime * result
				+ (this.getEdges() == null ? 0 : this.getEdges().hashCode());
		return result;
	}
